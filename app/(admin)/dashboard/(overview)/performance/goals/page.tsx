...
import { getAllGoals } from "@/app/lib/admin/performance/data";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
              <Target className="w-6 h-6 text-blue-600" /> Company Objectives &
              Goals
            </h1>
        ...
        >
          <Plus className="w-4 h-4" /> Assign New Goal
        </Link>
      </div>
            ...
            >
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  ...
                  <span
                    className={`px-2 py-1 rounded-md text-[10px] font-bold border capitalize ${goal.status === "Completed" ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-slate-50 text-slate-600 border-slate-200"}`}
                  >
                    {goal.status}
                  </span>
                </div>
