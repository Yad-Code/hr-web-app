...
export default async function MeetingDetailPage({
  ...
  // 1. Data fetcher validates 'view_dashboard' access internally
  const meeting = await getMeetingDetailsById(id);
  if (!meeting) notFound();

  // 2. Dynamic ABAC check: Can they MUTATE this meeting?
  ...
  return (
    <main className="max-w-5xl mx-auto w-full p-4 sm:p-6 lg:p-8 space-y-6 animate-fadeIn">
      <Link
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border capitalize tracking-wide ${getStatusBadge(meeting.status)}`}
            >
              {meeting.status}
            </span>
        ...
        {/* 👇 ABAC Gate: Only render management buttons if authorized */}
        {canManage && (
          <div className="flex items-center gap-2">
            {meeting.status.toLowerCase() !== "completed" && (
              ...
              >
                <button
                  type="submit"
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl hover:bg-emerald-100 transition-colors shadow-xs cursor-pointer"
                >
            ...
            {meeting.status.toLowerCase() !== "cancelled" && (
              ...
              >
                <button
                  type="submit"
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-rose-700 bg-rose-50 border border-rose-200 rounded-xl hover:bg-rose-100 transition-colors shadow-xs cursor-pointer"
                >
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-3">
            <h2 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <FileText className="w-4 h-4 text-purple-600" /> Meeting Agenda &
              Notes
            </h2>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-3">
            <h2 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <ListChecks className="w-4 h-4 text-indigo-600" /> Action Items &
              Follow-ups
            </h2>
