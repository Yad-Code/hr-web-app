// @/app/(admin)/dashboard/_components/quick-operations.tsx
...
export async function QuickOperationsWidget() {
  ...
  const { isLeader, pendingRequests } = await getQuickOperationsData(
    session.user.id,
  );

  return ( 
    <div className="bg-white border border-slate-100 rounded-2xl shadow-sm flex flex-col h-105 w-full overflow-hidden">
      {isLeader && <AutoRefresh intervalMs={10000} />}

      {/* Header stays fixed at the top (shrink-0 prevents it from squishing) */}
      <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
        ...
        {isLeader && pendingRequests.length > 0 && (
          <span className="flex items-center justify-center px-2 py-0.5 min-w-6 h-6 rounded-full bg-rose-100 text-rose-600 text-[10px] font-bold animate-fadeIn">
