  ...
  try {
    const [headcountRes, openPosRes, pendingRes, attendanceRes] =
      await Promise.all([
        ...
        SELECT COUNT(*) FROM users u WHERE u.status = 'Active' AND u.id IN (SELECT id FROM auth_users)
      `,
        db`SELECT COUNT(*) FROM job_postings WHERE status = 'Open'`,
        db`
        ${authUsersCTE(actorId, "approve_leaves")}
        SELECT COUNT(*) as total FROM leave_requests lr 
        WHERE lr.status ILIKE 'pending' AND lr.user_id IN (SELECT id FROM auth_users)
      `,
        db`
        ${authUsersCTE(actorId)}
        , target_users AS (
        ...
        scheduled_days AS (
          ...
          LEFT JOIN leave_requests lr 
            ON lr.user_id = e.user_id AND lr.type = 'dayoff' AND lr.status = 'Approved' AND e.date BETWEEN lr.start_date AND lr.end_date
          WHERE lr.id IS NULL
        ),
...
export async function getQuickOperationsData(actorId: string) {
  try {
    const permCheck = await db`
      ...
      WHERE up.user_id = ${actorId}::uuid AND p.action = 'approve_leaves' LIMIT 1
    `;
    const isLeader = permCheck.length > 0;
...
export async function getSecondaryWidgetsData(actorId: string) {
  try {
    const [outToday, complianceAlerts, payrollRun] = await Promise.all([
      ...
      `,
      db<DashboardPayrollRun[]>`
        SELECT pay_period_start, pay_period_end, status, pay_date
        FROM pay_stubs ORDER BY pay_period_start DESC LIMIT 1
      `,
    ]);
    ...
    return { outToday: [], complianceAlerts: [], payroll: null };
  }
}
