...
export async function verifyAccess(
  ...
  try { 
    const perms = await db`
      ...
      WHERE up.user_id = ${actorId} AND p.action = ${actionName}
    `;
    ...
    const targetQuery = await db`
      SELECT branch, department, manager_id FROM users WHERE id = ${targetUserId}
    `;
    if (targetQuery.length === 0) return false;
    const targetUser = targetQuery[0];
 
    const actorQuery = await db`
      SELECT branch, department FROM users WHERE id = ${actorId}
    `;
